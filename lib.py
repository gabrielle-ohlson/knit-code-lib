...
# --- CASTONS ---
# ---------------
def altTuckCaston(k, start_n, end_n, c, bed, gauge=1, inhook=False, releasehook=False, tuck_pattern=False, speedNumber=None, stitchNumber=None):
	'''
	for sheet caston
	...
	* `stitchNumber` (int, optional): value to set for the x-stitch-number knitout extension. Defaults to None.
	...
	if bed == 'f':
		...
		for n in needle_range2:
			if n % gauge == 0 and (((n/gauge) % 2) != 0):
				k.knit(dir2, f'f{n}', *cs)
			elif n == end_n: k.miss(dir2, f'f{n}', *cs)
	else:
		for n in needle_range1:
			if (gauge == 1 or n % gauge != 0) and ((((n-1)/gauge) % 2) == 0):
				k.knit(dir1, f'b{n}', *cs)
			elif n == start_n: k.miss(dir1, f'b{n}', *cs)
